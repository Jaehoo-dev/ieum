...
export function DetailedSelfFields({ member }: Props) {
  return (
    <div className="flex w-1/2 flex-col gap-0.5">
      ...
      <div>음주: {member.isDrinker ? "예" : "아니요"}</div>
      <div>음주: {member.alcoholConsumption}</div>
      ...
      <div>자산 관리: {member.assetManagementApproach}</div>
      <div>취미/관심사: {member.hobby}</div>
      <div>책: {member.bookTaste}</div>
      <div>여가: {member.leisureActivity}</div>
      <div>형제: {member.siblings}</div>
      <div>특징: {member.characteristics}</div>
      <div>10년 뒤: {member.tenYearFuture}</div>
      ...
      <div>자차: {member.hasCar ? "예" : "아니요"}</div>
      <div>
        게임: {member.doesGame ? "예" : "아니요"} {member.gameType}
      </div>
      <div>반려동물: {member.hasPet ? "예" : "아니요"}</div>
