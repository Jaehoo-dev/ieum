...
export function PastMegaphoneMatchesAsSenderSectionResolved() {
  ...
  const [pastMegaphoneMatchesAsSender] =
    api.megaphoneMatchRouter.findPastMatchesAsSenderByMemberId.useSuspenseQuery(
      { memberId: member.id },
    );
  const { sendMessage } = useSlackNotibot();

  return pastMegaphoneMatchesAsSender.length > 0 ? (
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="grid grid-cols-2 gap-3">
        {pastMegaphoneMatchesAsSender.map((match) => {
          return (
