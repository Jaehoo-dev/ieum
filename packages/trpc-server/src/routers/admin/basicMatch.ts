...
export const adminBasicMatchRouter = createTRPCRouter({
  findAll: protectedAdminProcedure
    ...
    .query(({ ctx, input: { statuses, name, from, to } }) => {
      ...
      return ctx.db.basicMatch.findMany({
        where: {
          AND: [
            {
  ...
  create: protectedAdminProcedure
    ...
    .mutation(
      async ({ ctx, input: { member1Id, member2Id, initialStatus } }) => {
        return ctx.db.basicMatch.create({
          data: {
  ...
  updateStatus: protectedAdminProcedure
    ...
    .mutation(({ ctx, input }) => {
      return ctx.db.basicMatch.update({
        where: {
          id: input.id,
        },
        data: {
  ...
  rejectBy: protectedAdminProcedure
    ...
    .mutation(async ({ ctx, input: { actionMemberId, matchId } }) => {
      const match = await ctx.db.basicMatch.findUniqueOrThrow({
        where: {
          id: matchId,
        },
        include: {
  ...
  delete: protectedAdminProcedure
    ...
    .mutation(({ ctx, input }) => {
      return ctx.db.basicMatch.delete({
        where: {
          id: input.id,
        },
      });
