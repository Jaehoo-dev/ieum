...
export const adminBasicMemberRouter = createTRPCRouter({
  create: protectedAdminProcedure
    ...
    .mutation(({ ctx, input }) => {
      return ctx.db.basicMember.create({
        data: input,
      });
    }),
  infiniteFindByGender: protectedAdminProcedure
    ...
    .query(async ({ ctx, input: { gender, limit, cursor } }) => {
      const members = await ctx.db.basicMember.findMany({
        take: limit + 1,
        where: {
          gender,
  ...
  findById: protectedAdminProcedure
    ...
    .query(({ ctx, input: { id } }) => {
      return ctx.db.basicMember.findUniqueOrThrow({
        where: {
          id,
        },
        include: {
  ...
  searchByName: protectedAdminProcedure
    ...
    .query(({ ctx, input: { name } }) => {
      return ctx.db.basicMember.findMany({
        where: {
          name,
        },
        include: {
  ...
  findMatchCandidates: protectedAdminProcedure
    ...
    .query(async ({ ctx, input: { id } }) => {
      const self = await ctx.db.basicMember.findUniqueOrThrow({
        where: {
          id,
          status: MemberStatus.ACTIVE,
        },
  ...
  addToBlacklist: protectedAdminProcedure
    ...
    .mutation(async ({ ctx, input: { actionMemberId, targetMemberId } }) => {
      return ctx.db.basicBlacklistItem.create({
        data: {
  ...
  inactivate: protectedAdminProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input: { id } }) => {
      return ctx.db.basicMember.update({
        where: {
          id,
        },
        data: {
  ...
  createProfile: protectedAdminProcedure
    ...
    .mutation(async ({ ctx, input: { memberId, profile } }) => {
      return ctx.db.basicMemberProfile.create({
        data: {
          ...profile,
          member: {
            connect: {
